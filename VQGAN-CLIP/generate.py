...
#default_image_size = 600  # >8GB VRAM
...
vq_parser.add_argument("-cd",   "--cuda_device", type=str, help="Cuda device to use", default="cuda:3", dest='cuda_device')
...
clip_loss = CLIPLoss(512).to(device)
...
def vector_quantize(x, codebook):
    ...
    indices = d.argmin(-1)
    x_q = F.one_hot(indices, codebook.shape[0]).to(d.dtype) @ codebook # [1, 37, 37, 256]
...
#scheduler = ReduceLROnPlateau(opt,'min',factor=0.999,patience=1000)
...
def synth(z):
    ...
    #print('----------------------------------')
    #print('z_q shape: '+str(z_q.shape)) # [1, 256, 37, 37]
...
def checkin(i, losses):
    ...
    loss_index = i
    for e in range(i):
        if loss_indices[e] < loss and e % args.display_freq == 0:
            loss = loss_indices[e]
            loss_index = e
    if i > 0:
        tqdm.write(f'last best image i: {loss_index}, loss: {round(loss,5)}')
    out = synth(z)
...
def ascend_txt():
    global i
    out = synth(z) # [1, 3, 592, 592]
    iii = perceptor.encode_image(normalize(make_cutouts(out))).float() # [32, 512]
    result = []
    if args.init_weight:
        # result.append(F.mse_loss(z, z_orig) * args.init_weight / 2)
        targets = torch.zeros_like(z_orig)
        #loss = F.mse_loss(z, targets)
        loss =F.smooth_l1_loss(z, targets,beta=0.6)
        weight = ((1/torch.tensor(i*2+1))*args.init_weight)/2
        l2_lambda = 1e-6
        l2_reg = torch.tensor(0.0).to(device)
        for name,param in model.named_parameters():
            if 'bias' not in name:
                l2_reg += (l2_lambda * torch.sum(torch.pow(param,2)))/2
        result.append(loss * weight + l2_lambda*l2_reg)
    '''
    loss = 0.0
    image = out.to(device)
    for prompt in args.prompts:
        text = clip.tokenize(prompt).to(device)
        loss += clip_loss(image, text)
    '''
...
def train(i):
    ...
    lossAll = ascend_txt()
    loss = sum(lossAll)
    loss_indices.append(loss.item())
    avg_losses = sum(loss_indices[i-100:])/len(loss_indices[i-100:]) if i>100 else \
                 sum(loss_indices)/len(loss_indices)
    ...
    with torch.inference_mode():
        z.copy_(z.maximum(z_min).minimum(z_max)) # [1, 256, 64, 64]
